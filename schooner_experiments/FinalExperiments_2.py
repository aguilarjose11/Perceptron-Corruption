...
Upper bounds per dimension to use when using a synthetic dataset. Shall be a list of float values.
    ...
    parser.add_argument('--train_fraction', type=float, help='Fraction of whole dataset to use as training.', default=0.2)
...
def obtain_data(args):
    ...
    if experiment == 'syn-lin': # Synthetic data experiment
        ...
        data = pd.read_csv('./datasets/skinClean.csv')
        skin = data
        if args.bias:
            skin['bias'] = 1

        data = data.drop('Class', axis=1).to_numpy()
        labels = skin.Class.to_numpy()
    else:
...
if __name__ == '__main__':
    ...
    # If within (0., 1.], then, it is a fraction. Else, it is a solid # of datapoints
    train_amount = args.train_fraction if 0. < args.train_fraction <= 1. else int(args.train_fraction)
    history = perceptron_corruption_experiment(
        X               = data,
        y               = labels,
        test_size       = args.test_fraction,
        train_size      = train_amount if real_dataset else None,
